...
class MySQLQuerySelectTest extends DBBaseTest {
    ...
    public function testSelectWhereAllChar() {
        ...
        try {
            // 一旦登録
            $dbConnection->queryAction("INSERT INTO test (column1, column2, column3, column4) VALUES (?, '2003-01-01 00:11:22', 12345, true)", ['あいうえおabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!"#$%&\'()-=^~\\@`[{;+:*]},<.>/_']);
            // SELECT実行
            $selectRet = $dbConnection->queryFetch($sql, $sqlParams);
        } catch (Exception $ex) {
            // ロールバック
    ...
    public function testSelectWhereLike() {
        ...
        $querySelect = DBFactory::createSelect(self::$mysqlDBName)
                ...
                ->_or()
                ->condition('column1', 'like', DBFactory::createDBHelper(self::$mysqlDBName)->escapeLikeValue('%_!\"#$%&\'()-=^~\\|@`[{;+:*]},<.>/_'))
                ->close()
                ->orderByAsc('column2')
        ;
        ...
        try {
            ...
            $dbConnection->queryAction("INSERT INTO test (column1, column2, column3, column4) VALUES ('value1', '2001-01-01 00:11:22', 12345, true)");
            $dbConnection->queryAction("INSERT INTO test (column1, column2, column3, column4) VALUES ('value2', '2002-01-01 00:11:22', 12345, true)");
            $dbConnection->queryAction("INSERT INTO test (column1, column2, column3, column4) VALUES (?, '2003-01-01 00:11:22', 12345, true)", ['%_!\"#$%&\'()-=^~\\|@`[{;+:*]},<.>/_']);
            // SELECT実行
            $selectRet = $dbConnection->queryFetch($sql, $sqlParams);
        } catch (Exception $ex) {
            // ロールバック
    ...
    public function testSelectNoLock() {
        ...
        $dbConnection = DBFactory::getConnection(self::$mysqlDBName);
        $dbConnectionOther = DBFactory::getConnection(self::$mysqlDBName, true);
        ...
        try {
            ...
            $this->assertTrue(DBFactory::createDBHelper(self::$mysqlDBName)->isErrorForLock($ex));
        }
    }
